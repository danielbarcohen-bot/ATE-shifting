...
from utils import get_moves_and_moveBit, calculate_ate_linear_regression_lstsq, \
    apply_data_preparations_seq


class ProbeATESearchNoHash(ProbeATESearch):
        ...
        while pq:
            ...
            if len(sequence) < max_seq_length:
                for func_name, col, move_bit in fast_moves:
                    ...
                    if any(new_df.equals(seen) for seen in seen_dfs):
                        continue

                    # if df hasnt been explored:
                    else:
                        # print(f"added func {func_name}", flush=True)
                        seen_dfs.append(new_df)
